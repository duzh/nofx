...
func fetchMarketDataWithStrategy(ctx *Context, engine *StrategyEngine) error {
	...
	for _, pos := range ctx.Positions {
		data, err := market.GetWithTimeframesExchange(pos.Symbol, engine.exchange, timeframes, primaryTimeframe, klineCount)
		if err != nil {
	...
	for _, coin := range ctx.CandidateCoins {
		...
		data, err := market.GetWithTimeframesExchange(coin.Symbol, engine.exchange, timeframes, primaryTimeframe, klineCount)
		if err != nil {
