  ...
  return (
    <div className="nofx-terminal" style={{ minHeight: '100vh', padding: 0 }}>
      ...
      >
        ...
        {!on &&
          ...
          !status.safe_mode && (
            ...
            >
              ...
              <span style={{ color: 'var(--tm-ink-2)' }}>
                It reads the whole market before acting — the first decision
                usually lands within a minute or two and will appear in the
                Execution Log below. You can stop it anytime from the Config
                page.
              </span>
            </div>
          )}
        ...
        {/* instrument switcher: one chip per held position + this cycle's
            candidates, driving every symbol-scoped panel below. The signal
            matrix can also switch, but it is empty without paid vergex data,
            so held symbols need their own always-visible entry point. */}
        <div
          className="tm-sc"
          style={{
            display: 'flex',
        ...
        >
          ...
          {(() => {
            ...
            for (const s of [
              ...
              const b = baseLabel(s)
              if (b && !chips.includes(b)) chips.push(b)
            }
            return chips.map((b) => {
              const isActive = b === activeSym
              const isHeld = heldSet.has(b)
              return (
                <button
                  key={b}
                  type="button"
                  onClick={() => setSelectedSym(b)}
                  style={{
                    padding: '1px 8px',
                    fontSize: 10,
        ...
        >
          ...
          <div style={sc}>
            ...
            >
              ...
              <span className="tm-sc">
                Recent closes · symbol/side/hold/pnl
              </span>
            </div>
        ...
        >
          <div style={{ ...sc, borderRight: cellBorder }}>
            ...
            >
              ...
              <span className="tm-sc">
                Market net inflow · {flow?.data?.window || '1h'} · Vergex
              </span>
              <span className="tm-sc" style={{ marginLeft: 'auto' }}>
